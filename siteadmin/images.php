...
if ($type == 1 || $type == 2 || $type == 3 || $type == 5 || $_GET['action'] == 99) {  //99 is in search menu (i.e. resetting the search page if link is clicked)
    unset($_SESSION["search_query_images"]);
}
...
if (isset($_POST['delete']) && isset($_POST['list'])) {
    ...
    foreach ($_POST['list'] as $image_id) {
        manageimages($image_id,'delete');
    }
    //notifications
    $show_notification = 1;
...
if (isset($_GET['delid'])) {
    ...
    $image_id = mysql_real_escape_string($_GET['delid']);
    manageimages($image_id,'delete');
    //notifications
    $show_notification = 1;
    $message = notifications(1);
...
if (isset($_POST['unfeature']) && isset($_POST['list'])) {
    //delete each posted video
    foreach ($_POST['list'] as $image_id) {
        manageimages($image_id,'unfeature');
    }
    //notifications
    $show_notification = 1;
    $message = notifications(1);
...
if (isset($_POST['reset_flags']) && isset($_POST['list'])) {
    //delete each posted video
    foreach ($_POST['list'] as $image_id) {
        manageimages($image_id,'reset_flags');
    }
    //notifications
    $show_notification = 1;
    $message = notifications(1);
...
if ($show_t == 1) {//active videos
    $pagination = pagination("SELECT * FROM images WHERE approved='yes' $sort",$limit);
}
if ($show_t == 2) {//Video approval
    $pagination = pagination("SELECT * FROM images WHERE approved='pending' $sort",
        $limit);
}
if ($show_t == 3) {//featured
    $pagination = pagination("SELECT * FROM images WHERE featured='yes' AND approved='yes' $sort",
        $limit);
}

if ($show_t == 5) {//flagged
    $pagination = pagination("SELECT * FROM images WHERE flag_counter > 0 AND approved='yes' $sort",
        $limit);
}
...
if ($show_t == 4 && !isset($_POST['search'])) {//sorting for top menu
    $search_query = $_SESSION["search_query_images"];
    $sql = "SELECT * FROM images WHERE $search_query";
    $pagination = pagination("SELECT * FROM images WHERE $search_term",$limit);
}
...
if (isset($_POST['search'])) {
    ...
    foreach ($_POST as $key => $value) {
        ...
        if ($key == 'title' || $key == 'gallery_name' || $key == 'user_id' || $key == 'indexer') {
            ...
            if ($value != '') {
                ...
                if ($key == 'gallery_name') {
                    $q1 = $key.' =\''.mysql_real_escape_string(trim($value)).'\' AND ';//search using LIKE
                }
    ...
    $search_query = $search_query.' AND approved = \'yes\'';	
    	...
    	$search_query = 'image_id = 100000000000000000000000000000001';	//quick fix
}
    $sql = "SELECT * FROM images WHERE $search_query";
    $pagination = pagination("SELECT * FROM images WHERE $search_term",$limit);
    //record search string to session for use later
    @session_start();
    @session_register("search_query_images");
    $_SESSION["search_query_images"] = $search_query;
}
...
if ($show_t == 1) {//active videos
    $sql = "SELECT * FROM images WHERE approved='yes' $sort LIMIT $set_limit, $limit";
}
if ($show_t == 2) {//Video approval
    $sql = "SELECT * FROM images WHERE approved='pending' $sort LIMIT $set_limit, $limit";
}
if ($show_t == 3) {//featured
    $sql = "SELECT * FROM images WHERE featured='yes' AND approved='yes' $sort LIMIT $set_limit, $limit";
}

if ($show_t == 5) {//flagged
    $sql = "SELECT * FROM images WHERE flag_counter > 0 AND approved='yes' $sort LIMIT $set_limit, $limit";
}

if ($show_t == 4 && !isset($_POST['search'])) {//sorting for top menu
    //search option using session information
    $search_query = $_SESSION["search_query_images"];
    $result_sql = "SELECT * FROM images WHERE $search_query LIMIT $set_limit, $limit";
}
...
////////////////////////////////////////////
$url = 'images.php';//the url to be put in links - EDIT ME
...
$inner_template1 = "templates/inner_images.html";//middle of page
$TBS = new clsTinyButStrong;
$TBS->NoErr = true;// no more error message displayed.
$TBS->LoadTemplate("$template");
$TBS->MergeBlock('blkallimages',$result_active);
