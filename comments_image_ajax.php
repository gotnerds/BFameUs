...
$text_comments	= 0;
$sql_count		= "SELECT image_id FROM imagecomments WHERE image_id = $image_id";
...
while ($result = @mysql_fetch_array($query)) {
	...
	//______PERMISSIONS CHECK COMMENTS______________________________________________________________
    	...
    	$image_comment_edit_on = '<a href="edit/editcomments.php?type=6&id='.$indexer_id.'" title="Edit" onclick="return parent.GB_show(\'Edit Comment\', this.href, 400, 700)">
    	...
    	$image_comment_delete_on = '<a href="edit/delete.php?type=7&id='.$indexer_id.'" title="Delete" onclick="return parent.GB_show(\'Delete Comment\', this.href, 300, 700)">
	...
	// we need to query imagecomments_replys to find any REPLIES
	...
	while ($result_replies = @mysql_fetch_array($replies)) {
		...
		if ( $reply_delete_on == 1 ) {
			$delete_reply_on = "<div style=\"margin-right:12px; float:right;\">
			  <a href=\"edit/delete.php?type=16&id=$reply_id\" title=\"$lang_delete_general\" onclick=\"return parent.GB_show('Delete Reply', this.href, 350, 550)\">
		...
		if ( $reply_edit_on == 1 ) {
			$edit_reply_on = "<div style=\"margin-right:12px; float:right;\">
			  <a href=\"edit/editcomments.php?type=7&id=$reply_id\" title=\"$lang_edit\" onclick=\"return parent.GB_show('Edit Reply', this.href, 350, 550)\">
...
$template = "themes/$user_theme/templates/inner_comments_image_ajax.htm";
